...
typedef struct{
    short ex; //esiste il carattere nella parola di riferimento
    char *no; //posizioni in cui il carattere non può stare 
    unsigned short esatto; //numero esatto del carattere nella parola di riferimento
    unsigned short min; //numero minimo del carattere nella parola di riferimento
...
typedef struct{
    unsigned short num; //n° di volte che quella lettera compare nella parola di riferimento
    unsigned short cont; //utile dopo per capire quali parole c'è da filtrare
}scan; 
...
void init_diz(char *ver){
    for(unsigned short i = 0; i<DIZ;i++){
        if(i>=45){
            diz[i].esatto=0;
            diz[i].min=0;
            diz[i].ex = -1;
...
int presente(char c, char *s, unsigned short *conteggio){
    unsigned short i = 0;
...
int validazione(char *conf, char * ver, char * rif){
    unsigned short conteggio = 0;
    unsigned short conto = 0;

    for(int i = 0;i<k;i++){
        unsigned short register num = (int) conf[i]; //uso il register per mantenere la variabile in un registro della cpu così c'è l'ha sempre pronta
        unsigned short register num2 = (int) rif[i];
...
unsigned short uguale(char *c, char *p){
    for(unsigned short i = 0;i<k;i++)
        if(c[i] != p[i]) return 0;
    return 1;
}
short posizione(char* c1, char *c2){ //c2 la testa
    for(unsigned short i = 0; i<k;i++){
...
void insert(char *stringa, NodePtr *radicec, unsigned short validazione, Root *root, unsigned short conf) {
        ...
        char *st = calloc(1,sizeof(char)*k); 
...
void stampa_lista_filtrato(NodePtr l,char *ver, char *rif,unsigned short i){
    while(l!=NULL) {
...
void conto_ordinata_filtrato(NodePtr x,char *ver, char *rif,unsigned short i, unsigned short int ins){
    if(x!=TNULL){  
...
int main(void){
    ...
    //rif[0] = '&';
    unsigned short nuova = 0; //mi identifica inizio di una partita
    unsigned short inserimento = 1; //mi identifica se devo fare un inserimento di nuove patole
    unsigned short conteggio = 0;  //mi dice con quante parole devo confrontare la massimo 
    unsigned short primo_inserimento = 0; //letto la prima parola da confrontare
    unsigned short confronto_fatto = 0; //il confronto è stato fatto
